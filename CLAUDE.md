...
curl -fSL -o /tmp/go1.25.10.linux-amd64.tar.gz "https://go.dev/dl/go1.25.10.linux-amd64.tar.gz"

# Install (removes any previous Go installation in /usr/local/go)
rm -rf /usr/local/go && tar -C /usr/local -xzf /tmp/go1.25.10.linux-amd64.tar.gz

# Verify
go version   # should print "go version go1.25.10 linux/amd64"
```
