...
func (p *Proxy) handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	proxyReq, err := http.NewRequestWithContext(r.Context(), r.Method, upstreamURL.String(), strings.NewReader(string(reqBody)))
	if err != nil {
	...
	// Use default transport which respects proxy env vars.
	client := &http.Client{
...
func (p *Proxy) shouldCapture(path string) bool {
	...
	matched, _ := filepath.Match(p.config.Filter, path)
	return matched
}
...
func (p *Proxy) generateFilename(method, path string, status int) string {
	...
	case "skip":
		// Check if a file with this base exists.
		matches, _ := filepath.Glob(filepath.Join(p.config.WriteDir, base+"_*.yaml"))
		if len(matches) > 0 {
